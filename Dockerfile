...
RUN apk --no-cache add --virtual build-dependencies make libevent-dev openssl-dev gcc libc-dev  \
 ...
 && chmod u=x,go= /usr/local/bin/chown2root /usr/local/bin/mkdir2root \
 && mkdir -p "$CONFIG_DIR" "$UNIX_SOCKET_DIR" \
 && chown pgbouncer "$CONFIG_DIR" "$UNIX_SOCKET_DIR" \
 && echo "pgbouncer ALL=(root) NOPASSWD: /usr/local/bin/chown2root, /usr/local/bin/mkdir2root" > /etc/sudoers.d/pgbouncer
